...
- A. Training datasets and clothing conditions: 58 scores with a shared target/condition header. Each training dataset appears once. Repeated reports for a method, exact training/test dataset labels, input and metric are consolidated by their maximum; superscripts preserve reporting-source attribution. CASIA-B and CASIA-B* are not pooled. BiggerGait Table 1 supplies the main CCPG and CCGR-Mini comparisons; the higher GaitBase SUSTech1K overall/CL scores (17.3/9.7) come from DenoisingGait supplementary Table 9; GaitMax contributes its own two CL scores without repeating the four BigGait / DenoisingGait baseline scores already shown. DenoisingGait supplementary Table 9 supplies alternative-source CL scores and the two corresponding SUSTech1K overall scores.
...
`data/cross-domain-table.json` identifies the selected source record and metric for every cell, as well as candidate record IDs for maximum selection. The explorer continues to show individual reporting sources, preserving all original values. The compact table places CCPG training on the left, CCGR-Mini and the other training directions on the right, and the silhouette/pose comparisons underneath. The older `data/cross-domain-figure.json` is retained as the provenance of the former figure; it does not define the current layout.
